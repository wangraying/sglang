...
class Scheduler:
        ...
        if (
            ...
            if not self.last_batch.is_empty():
                if self.running_batch is None:
                    ...
                    logger.debug(f"Merge prefill batch into running batch, size: {self.running_batch.batch_size()}, output_ids: {self.running_batch.output_ids}, seq_lens: {self.running_batch.seq_lens}")

        # Prefill first
        new_batch = self.get_new_batch_prefill()
        if new_batch is not None:
            logger.debug(f"Prefill new batch, size: {new_batch.batch_size()}, input_ids: {new_batch.input_ids}, seq_lens: {new_batch.seq_lens}, prefix_lens: {new_batch.prefix_lens}, extend_lens: {new_batch.extend_lens}")
            return new_batch
    ...
    def run_batch(self, batch: ScheduleBatch):
        ...
        if self.is_generation:
            ...
            logger.debug(f"Run batch #{model_worker_batch.bid}, mode={batch.forward_mode}, input_ids={model_worker_batch.input_ids}, output_ids={next_token_ids}")
